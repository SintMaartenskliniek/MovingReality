...
filename = "./meanFSA.xlsx"
FSAdata <- read_excel(filename)

# change names
FSAdata$trialtype[(FSAdata$trialtype) == "2Reg"] <- "Reg2"
FSAdata$trialtype[(FSAdata$trialtype) == "2FB"] <- "FBz2"

# The basemodel only takes the random intercept of participantID into account on the outcome FSA
...
basemodel = lmer(FSA ~ trialtype + (1|participantID), data=FSAdata, REML=FALSE, na.action=na.exclude) # Only take the random intercept based on participant ID into account as random variable on outcome 'FSA'


# Add group (trial order) as fixed variable (including interaction effect) to the model
# Evaluate the model by maximum likelihood, not restricted maximum likelihood (REML), so set REML to FALSE (will allow the "anova" function later)
model_1 = lmer(FSA ~ trialtype * group + (1|participantID), data=FSAdata, REML=FALSE, na.action=na.exclude)
...
# Pr(>Chisq) = 0.0215; and decrease of AIC > continue with model_1 (adding group as a fixed parameter improves the accuracy of the model)


# Add time as a fixed effect (to estimate learning effect)
model_2 = lmer(FSA ~ time + trialtype * group + (1|participantID), data=FSAdata, REML=FALSE, na.action=na.exclude)
...
model_3 = lmer(FSA ~ trialtype * group + (1+time|participantID), data=FSAdata, REML=FALSE, na.action=na.exclude)

anovaResult_3 <- anova(model_1, model_3)
print(anovaResult_3)

# Pr(>Chisq) = 0.0977; continue with model_1
...
#   * Main effect of trialtype (trialtype effects are all significant), meaning the trialtype has an effect on the mean FSA
#   * Main group effect (group effect is significant), meaning there is a different intercept between the groups (group that received feedback on propulsion has a higher FSA compared to the group that first received feedback on initial contact, independent for the trialtype)
...
# # Result suggest:
# #   *  Significant difference between FSA in 1Reg and FBIC conditions
# #   *  Significant difference between FSA in 1Reg and FBPO conditions
# #   *  Significant difference between FSA in 1Reg and 2FB conditions
# #   *  Significant difference between FSA in 1Reg and 2Reg conditions
# #   *  No significant difference between FSA in FBIC and 2FB conditions
# #   *  Significant difference between FSA in FBIC and FBPO conditions
...
# df_wide <- pivot_wider(FSAdata, names_from = trialtype, values_from = FSA, id_cols=participantID)
# # Rename columns
...
# # FBIC - FB2
# # Perform paired t-tests, use confidence level of 95%
# ttest_FBIC_FB2 <- t.test(df_wide$FBIC, df_wide$FB2, paired = TRUE, conf.level=0.95)
# # Print the pairwise result
# print(ttest_FBIC_FB2)
# 
# 
# # FBIC - FBPO
# # Perform paired t-tests, use confidence level of 95%
# ttest_FBIC_FBPO <- t.test(df_wide$FBIC, df_wide$FBPO, paired = TRUE, conf.level=0.95)
# # Print the pairwise result
# print(ttest_FBIC_FBPO)
# 
# 
# # Place all p-values in one variable for Bonferroni correction
# p_values <- c(ttest_Reg1_FBIC$p.value, ttest_Reg1_FBPO$p.value, ttest_Reg1_FB2$p.value, ttest_Reg1_Reg2$p.value, ttest_FBIC_FB2$p.value, ttest_FBIC_FBPO$p.value)
...
# #   * Significant difference between FSA in 1Reg and 2Reg conditions
# #   * No significant difference between FSA in FBIC and 2FB conditions
# #   * No significant difference between FSA in FBIC and FBPO conditions
